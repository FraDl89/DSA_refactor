...
This solves the pde assuming that the infectious time distribution (-i) is gamma with parameters (-ip) shape=0.666, scale=13.5, the contact interval distribution (-c) is exponential with scale parameter (-cp) 0.26, and the initial condition (-r) is rho=0.002. The mesh (-d) both in dx and dt is of size 0.1, and the PDE is solved until the final time (-t) 130. Finally, all the outputs produced are saved with suffix (-o) "result". Distributions available at the moment are 'exponential', 'gamma', 'lognormal', 'weibull'. So, for instance, you can run the same algorithm with python pde_example.py -i lognormal -c gamma -r0.02 -t 130 -d 0.1 -ip 1 1 -cp 1 1 -o result. The order of the parameters can be found by looking at the scipy.stats documentation for the relative distribution.
...
The rest of the repo is as follows. In the main folder you can find the functions that are at the core of this repo.
	...
	3) Selkealgo.py is a version of the Sellke algorithm, used to generate epidemic curves from non-markovian epidemic models with the Sellke construction, alon with a fully worked out example, alon with a fully worked out example.

The code to reproduce numerics can be found under the folders named Syntetic_data, India, and foot_and_mouth. Results and Data are already stored under the subfolders named Data, except for India data which generates files too big for github to handle. 
