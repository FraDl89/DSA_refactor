    ...
    **kwargs: keywords arguments
    ...
    def ll_I(self,u_rec):
        ...
        if type(self.infect_times)==int:
            ...
            pdeObj_1,rec_times = self.pde(u_rec)

            
            X,Y,Yts=pdeObj_1.finDiffUpdate(Yts=True, initial_cond=self.initial_cond)

            
            xS = interp1d(pdeObj_1.tgrids, X)
            YtsI= interp1d(pdeObj_1.tgrids, Yts)
    ...
    def ll_R2(self,u_rec):
        ...
        if type(self.recov_times)==int:
            ...
            X,Y=pdeObj_1.finDiffUpdate(initial_cond=self.initial_cond)
            dx = pdeObj_1.dx
            ...
            gt = convolve(ft,rec_times)[1:len(pdeObj_1.tgrids)]*dx
            gt = gt/(np.sum(gt)*dx)
            
            loggt = np.log(gt)
            res=interp1d(pdeObj_1.tgrids[1:], loggt)(recov_times[recov_times>pdeObj_1.tgrids[1]]).sum()
            return res
    ...
    def minimize_likelihood(self,lb,ub,use_pyswarm=True):
        '''
        ...
        if use_pyswarm==True:
               xopt,fopt = pso(neg_log_likelihood,lb,ub,maxiter=200,swarmsize=100,minfunc=1e-4, debug=False)
